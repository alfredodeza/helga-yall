...
def is_getting_thanked(message, botnick=None):
    ...
    thanks_regex = r'(({botnick}:|{botnick})+\s+(is|you|you are)+\s+({nice_adjectives})+|(.*)(is|you are)+\s+({nice_adjectives})+\s+(({botnick}:|{botnick}))+|^({nice_adjectives})+\s+({botnick})+|^({nice_adjectives})+\s+({botnick}:|{botnick}))+'.format(
